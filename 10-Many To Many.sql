...
CREATE TABLE reviews (
    id INT AUTO_INCREMENT PRIMARY KEY,
    rating DECIMAL(2,1), -- maximum 9.9
    reviewer_id INT,
    series_id INT,
...
-- TV Joins Challenge 1 - ALL RATINGS FOR EACH SERIES
SELECT title, rating
FROM series INNER JOIN reviews ON series.id = reviews.series_id
ORDER BY title; 

-- TV Joins Challenge 2 - SERIES AVG RATINGS
SELECT title, AVG(rating) AS avg_rating
FROM series INNER JOIN reviews ON series.id = reviews.series_id
GROUP BY series.id 
ORDER BY avg_rating; 

-- TV Joins Challenge 3 - ALL RATINGS GIVEN BY EACH REVIEWER
...
SELECT first_name, last_name, 
        ...
        CASE
            ...
            ELSE 'INACTIVE' 
        END AS STATUS -- used when there are more than 2 conditions
        -- IF(COUNT(rating) > 0, 'ACTIVE', 'INACTIVE') AS STATUS
FROM reviewers LEFT JOIN reviews ON reviewers.id = reviews.reviewer_id
GROUP BY reviewers.id;

-- TV Joins Challenge 7 - 3 TABLES
